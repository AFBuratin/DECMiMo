...
compute_concordance_withbetw <- function(ps_fitted_list, maxrank){
  conc_df <- NULL
  for(i in 1:length(ps_fitted_list$test$pval)){ # i in 1:n comparisons
    # i = 1
    # pval extraction
    P_df1 <- ps_fitted_list$Heldout$pval[[i]] # first half data
    P_df2 <- ps_fitted_list$test$pval[[i]] # second half data
    rownames(P_df1) = P_df1$rn
    rownames(P_df2) = P_df2$rn
    
    lfc_df1 <- ps_fitted_list$test$lfc[[i]]
    lfc_df1[is.na(lfc_df1)] = 0
    lfc_df2 <- ps_fitted_list$Heldout$lfc[[i]]
    lfc_df2[is.na(lfc_df2)] = 0
    rownames(lfc_df1) = lfc_df1$rn
    rownames(lfc_df2) = lfc_df2$rn
    
    nmethods <- length(names(ps_fitted_list$test$pval[[i]]))
    for(j in 2:nmethods){ # j in method names
      # j = 3
      cat("Mehod", names(ps_fitted_list$test$pval[[i]])[j], "with") #,"with GLMM")
      # vec1_pdf1 = P_df1[lfc_df1[-abs(lfc_df1[,j]),"rn"],j]
      vec1_pdf1 = -P_df1[,j]
      # names(vec1_pdf1) <- lfc_df1[-abs(lfc_df1[,j]),"rn"]
      names(vec1_pdf1) <- P_df1[,"rn"]
      
      # vec1_pdf2 = P_df2[lfc_df2[-abs(lfc_df2[,j]),"rn"],j]
      vec1_pdf2 = -P_df2[,j]
      
      # names(vec1_pdf2) <- lfc_df2[-abs(lfc_df2[,j]),"rn"]
      names(vec1_pdf2) <- P_df2[,"rn"]
      
      for(k in 2:nmethods){ # k in method names again
        # k = 3
        cat("\t",names(ps_fitted_list$test$pval[[i]])[k],"\n")
        # vec2_pdf1 = P_df1[lfc_df1[-abs(lfc_df1[,k]),"rn"],k]
        vec2_pdf1 = -P_df1[,k]
        
        # names(vec2_pdf1) <- lfc_df1[-abs(lfc_df1[,k]),"rn"]
        names(vec2_pdf1) <- P_df1[,"rn"]
        
        # vec2_pdf2 = P_df2[lfc_df2[-abs(lfc_df2[,k]),"rn"],k]
        vec2_pdf2 = -P_df2[,k]
        # names(vec2_pdf2) <- lfc_df2[-abs(lfc_df2[,k]),"rn"]
        names(vec2_pdf2) <- P_df2[,"rn"]
...
compute_concordance_withGLMM <- function(ps_fitted_list, maxrank){
  ...
  for(i in 1:length(ps_fitted_list$test$pval)){ # i in 1:n comparisons
    ...
    for(j in 2:nmethods){ # j in method names
      ...
      vec1 = P_df1[lfc_df1$rn[-abs(lfc_df1[,j])],j]
      names(vec1) <- P_df1[lfc_df1$rn[-abs(lfc_df1[,j])],"rn"]
      vec2 = P_df2[lfc_df2$rn[-abs(lfc_df2[,"GLMM"])],"GLMM"]
      names(vec2) <- P_df2[lfc_df2$rn[-abs(lfc_df2[,"GLMM"])],"rn"]
      # for(k in 1:nmethods){ # k in method names again
      # k = 1
...
gheat <- function(AUC_AOC_between_methods,concordance_df_summary){
  # Filtering
  #gheat.list = list()
  #for(m in unique(AUC_AOC_between_methods$method1)){
  ...
  AUC_AOC_between_methods.F$method1 = factor(AUC_AOC_between_methods.F$method1,levels = unique(AUC_AOC_between_methods.F$method1), 
                                             labels = c(
                                               "DESeq2",
                                               ...
                                               "GLMM",
                                               "voom"), ordered = F)
  AUC_AOC_between_methods.F$method2 = factor(AUC_AOC_between_methods.F$method2,levels = unique(AUC_AOC_between_methods.F$method2), 
                                             labels = c(
                                               "DESeq2",
  ...
  g_vertical_dendrogram <- ggplot() + 
    ...
    theme(axis.line.y=element_blank(),
          ...
          plot.margin = unit(c(0.1,0.1,0.1,0.1), "cm")) +
    ggtitle(label = paste0("Dataset ", Dataset),
  ...
  a2 <- plot_grid(plotlist = list(g_horizontal_dendrogram,g_horizontal_dendrogram,

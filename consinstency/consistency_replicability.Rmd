...
AUC_AOC_between_methods <- suppressWarnings(ddply(concordance_df_summary, ~ method1 + method2, function(x) AucAocFun(cVals = x$concordance[x$rank <= 250], nfeatures = mean(x$nfeatures[x$rank <= 250]), threshold = 250)))
...
gheat <- function(AUC_AOC_between_methods,concordance_df_summary){
  ...
  # for(m in unique(AUC_AOC_between_methods$SetSize)){
    ...
    AUC_AOC_between_methods.F$method1 = factor(AUC_AOC_between_methods.F$method1, levels = c("DESeq2", "GLMM", "edgeR-robust","voom"),
                                labels = c("DESeq2", "GLMM", "edgeR","voom"))
    AUC_AOC_between_methods.F$method2 = factor(AUC_AOC_between_methods.F$method2, levels = c("DESeq2", "GLMM", "edgeR-robust","voom"),
                                labels = c("DESeq2", "GLMM", "edgeR","voom"))
    concordance_df_summary.F <- concordance_df_summary
    concordance_df_summary.F$method1 <- factor(concordance_df_summary.F$method1, levels = c("DESeq2", "GLMM", "edgeR-robust","voom"),
                                labels = c("DESeq2", "GLMM", "edgeR","voom"))
    concordance_df_summary.F$method2 <- factor(concordance_df_summary.F$method2, levels = c("DESeq2", "GLMM", "edgeR-robust","voom"),
                                labels = c("DESeq2", "GLMM", "edgeR","voom"))
    ...
    concordance_df_summary.F_area$method1 <- factor(concordance_df_summary.F_area$method1, levels = c("DESeq2", "GLMM", "edgeR","voom")[hc$order],
                                labels = c("DESeq2","GLMM", "edgeR","voom")[hc$order])
    concordance_df_summary.F_area$method2 <- factor(concordance_df_summary.F_area$method2)
    concordance_df_summary.F_area$method2 <- factor(concordance_df_summary.F_area$method2, levels = c("DESeq2", "GLMM","edgeR","voom")[hc$order],
                                labels = c("DESeq2","GLMM","edgeR","voom")[hc$order])
    # edges
    ...
    g_heat <- ggplot(concordance_df_summary.F_area,aes(x = rank, y = concordance)) +
      ...
      #              mapping = aes(x = 0, xend = 0, y = 0, yend = 1, color = "green")) +
      # 
      # geom_segment(concordance_df_summary.F_area[concordance_df_summary.F_area$method1 == concordance_df_summary.F_area$method2,],
      #              mapping = aes(x = 100, xend = 100, y = 1, yend = 0, color = "green")) +
      # geom_segment(concordance_df_summary.F_area[concordance_df_summary.F_area$method1 == concordance_df_summary.F_area$method2,],
      #              mapping = aes(x = 0, xend = 100, y = 1, yend = 1, color = "green")) +
      # geom_segment(concordance_df_summary.F_area[concordance_df_summary.F_area$method1 == concordance_df_summary.F_area$method2,],
      #              mapping = aes(x = 100, xend = 0, y = 0, yend = 0, color = "blue")) +
      #scale_fill_gradientn(colours = c("red","yellow","turquoise"),limits = c(-0.01,1)) +
      scale_fill_distiller(palette = "Reds",limits = c(0,1), direction = -1) +
...
gheat <- function(AUC_AOC_between_methods,concordance_df_summary){
 # Filtering
  ...
  # for(m in unique(AUC_AOC_between_methods$SetSize)){
    ...
    # Area extraction
    area <- apply(concordance_df_summary.F,1,function(x){
      area <- AUC_AOC_between_methods.F$conservArea[AUC_AOC_between_methods.F$method1 %in% x["method1"] & AUC_AOC_between_methods.F$method2 %in% x["method2"]]
      return(1-area)
    })
    concordance_df_summary.F_area <- cbind(concordance_df_summary.F,area = area)
    # As factor
...
conc_df_sub <- conc.betglmm.IPF[conc.betglmm.IPF$rank == 500 & conc.betglmm.IPF$subset == "HeldoutvsTest",]
conc_df_sub <- conc.witbet.IPF[conc.witbet.IPF$rank == 90 & conc.witbet.IPF$subset == "HeldoutvsTest",]

conc_df_sub <- conc.betglmm.ALZ[conc.betglmm.ALZ$rank == 1000 & conc.betglmm.ALZ$subset == "HeldoutvsTest",]
conc_df_sub <- conc.witbet.ALZ[conc.witbet.ALZ$rank == 100 & conc.witbet.ALZ$subset == "HeldoutvsTest",]
...
df2 <- data_summary(conc_df_sub, varname="concordance", 
                    groupnames=c("method1"))
# Convert dose to a factor variable
df2$method1=as.factor(df2$method1)
head(df2)
ggplot(df2, aes(x=method1, y=concordance, fill=method1)) + 
  geom_bar(stat="identity", color="black", 
           position=position_dodge()) +
  geom_errorbar(aes(ymin=concordance-sd, ymax=concordance+sd), width=.2,
                 position=position_dodge(.9)) 
my_comparisons <- list( c("DESeq2", "GLMM"),
                        c("edgeR", "GLMM"),
                        c("voom", "GLMM"))
conc_df_sub$method1 = factor(conc_df_sub$method1, levels = c("GLMM","DESeq2","edgeR","voom"), ordered = T)
# Box plots with p-values
library(ggpubr)
library(rstatix)
stat.test <- conc_df_sub %>%
  # group_by(supp) %>%
  t_test(concordance ~ method1)
stat.test
stat.test <- stat.test %>% add_y_position()
ggboxplot(conc_df_sub, x = "method1", y = "concordance", fill = "method1") +
  stat_pvalue_manual(stat.test[1:3,], label = "p", tip.length = 0.01) +
...
ggboxplot(conc_df_sub, x = "method1", y = "concordance",
          color = "method1")+ 
...
AUC_WM_df <- conc_df_sub[conc_df_sub$rank == 1000 & conc_df_sub$subset == "HeldoutvsTest",]
...
rank_WM_concordance$method <- factor(rank_WM_concordance$method, levels = levels(rank_WM_concordance$method), 
                              labels = c(
                                         ...
                                         "GLMM",
                                         "voom"))
rank_WM_concordance$value = rank(rank_WM_concordance$value)
...
det.method = ps_fitted$test$pval
  ...
  for(j in 1:nmethods){
    ...
    dec_list[[colnames(det.method[[i]])[j+1]]] = data.frame(decs = det.method[[i]][!is.na(det.method[[i]][,j+1]) & det.method[[i]][,j+1]<=0.1,"rn"])
...
decs_list_glmm <- data.frame(methods = "GLMM", decs = rownames(ps_fitted$Heldout$pval[[i]]["GLMM"])[!is.na(ps_fitted$Heldout$pval[[i]]["GLMM"]) & ps_fitted$Heldout$pval[[i]]["GLMM"]<=0.1])
  ...
  for(j in 1:nmethods){
    ...
    decsT_list[[colnames(ps_fitted$Heldout$pval[[i]])[j+1]]] = data.frame(decs = ps_fitted$Heldout$pval[[i]][!is.na(ps_fitted$Heldout$pval[[i]][,j+1]) & ps_fitted$Heldout$pval[[i]][,j+1]<=0.1,"rn"])

  }


names(decsT_list) = paste0(names(decsT_list),"-verification")
names(dec_list) = paste0(names(dec_list),"-evaluation")
...
decs_tabs <-  as.data.frame(xtabs(~methods+decs, data=rbind.fill(decs_rlist, decsT_rlist))) %>% reshape2::dcast(decs ~ methods, value.var = "Freq")
...
# }
  ...
  UpSetR::upset(decs_t,
    ...
    sets.x.label = paste0("DECs per DEMs"), 

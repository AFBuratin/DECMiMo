...
# write.csv(conc.witbet, "/blackhole/alessia/CircModel/consinstency/conc_witbet_IPF.csv")
...
# conc.betglmm.TALL <- compute_concordance_withGLMM(ps_fitted_list = ps_fitted, maxrank = maxrank)
# write.csv(conc.betglmm, "/blackhole/alessia/CircModel/consinstency/conc_betGLMM_IPF.csv")
write.csv(conc.betglmm.ALZ, "/blackhole/alessia/GLMM_article/consinstency/conc_betGLMMadj_ALZ_30.csv")
# write.csv(conc.betglmm.TALL, "/blackhole/alessia/CircModel/consinstency/conc_betGLMMadj_TALL_30.csv")
...
concordance_df_summary <- suppressWarnings(ddply(conc.witbet.ALZ, ~ rank + method1 + method2, function(x) colMeans(x[,c("concordance", "nfeatures")])))
...
gheat <- function(AUC_AOC_between_methods,concordance_df_summary){
  ...
  # for(m in unique(AUC_AOC_between_methods$SetSize)){
    ...
    g_legend_dendrogram <- get_legend(ggplot() +
                                        geom_point(data=forlegend, aes(x = method1, y = 1, color = method1),size = 4) +
    ...
    g_horizontal_dendrogram <- ggplot() +
      geom_segment(data=dendro_data(hc)$segments, aes(x=x, y=y, xend=xend, yend=yend)) +
      geom_point(data=dendro_data(hc)$labels, aes(x=x, y=y,color=label),size = 4) +
      scale_y_continuous() +
...
fig <- plot_grid(alz$heat,
                 plot_grid(alz$legend_dendro,alz$legend_heat,ncol = 2,rel_widths = c(1,0.4)),nrow = 2,rel_heights = c(1,0.1))
fig

png("/blackhole/alessia/GLMM_article/consinstency/figCATbetween_ALZdataset.png", width = 29.5, height = 20, res = 100, units = "cm")
fig
dev.off()
...
conc_df_sub <- conc.witbet.ALZ[conc.witbet.ALZ$rank == 400 & conc.witbet.ALZ$subset == "HeldoutvsTest",]
...
conc_df_sub$method1 <- factor(conc_df_sub$method1, levels = levels(conc_df_sub$method1), 
                              labels = c(
                                         ...
                                         "voom"))
                                         # "EBSeq"))
png(paste0("/blackhole/alessia/GLMM_article/consinstency/ConcordancewithGLMM_r30_top", maxrank, Dataset, ".png"), 

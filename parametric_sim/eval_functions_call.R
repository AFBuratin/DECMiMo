source("/blackhole/alessia/GLMM_article/parametric_sim/eval_functions.R")
...
evals <- bplapply(sims, FUN = function(sim){
  #sim = sims$`simulation: 1_dataset:findcirc_distribution:NB_sampleSize:5_FPR:0.1_foldEffect:1.5_seed:142180635`
...
evals.glmm <- list()
minutes <- c()
eval.glmm = foreach::foreach (i=c(1:30)) %dopar% {#seq(1,length(sims)-1, by = 8)){
  # i = 1
  cat("Simulation name:",names(sims.glmm2)[[i]],"\n")
  sim.glmm <- list()
  
  for (k in seq(1, 4, 1)){ #detection methods used for GLMM 
    # k = 1
...
for(i in c(1:30)){
  evals_all_glmm[[i]] = append(evals_all[[i]], list(evals.glmm[[i]]))  
  names(evals_all_glmm[[i]]) = c(names(evals_all[[i]]), "GLMM")
}
...
names.glmm = apply(simulation_flow.glmm, 1, function(sim) paste(colnames(simulation_flow.glmm),
                                                   sim,
                                                   sep = ":",
                                                   collapse = "_"))

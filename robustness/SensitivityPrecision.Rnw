...
getSensitivityAlgo <- function(m, alpha, alphaOut) {
  t(sapply(1:nreps, function(i) sapply(namesAlgosALL, function(algo) {
    sigHeldout <- resHeldout.adj[[m]][[i]][[algo]] <= alpha
    mean((resTes.adj[[m]][[i]][[algo]] <= alpha)[sigHeldout])
  })))
}

getPrecisionAlgo <- function(m, alpha, alphaOut) {
  t(sapply(1:nreps, function(i) sapply(namesAlgosALL, function(algo) {
    sigTest <- resTes.adj[[m]][[i]][[algo]] <= alpha #Evaluation set
    if (sum(sigTest) == 0) return(0)
    mean((resHeldout.adj[[m]][[i]][[algo]] <= alpha)[sigTest]) #TP rate
  })))
}
...
resTes.adj = lapply(res_GLM_GLMM_rep_Test, function(x){
  ...
  new.res = new.res %>% dplyr::rename(edgeR = `edgeR-robust`)
  return(new.res)})
...
d$algorithm <- renameAtoB(d$algorithm, "edgeR.robust", "edgeR")
...
getSensitivityAlgoGold <- function(alpha, alphaOut, gold) {
  ...
  wh.pred = resTes.adj[[i]][[algo]] <= alpha
  wh.pos = which(wh.pred)
  wh.neg = which(!wh.pred)
  wh.TP = which(resHeldout.adj[[i]][[gold]] <= alphaOut)
  ...
  # precision = if ((TPs + FPs) == 0) 0 else TPs/(TPs + FPs)
  Sensitivity
  })))
}
# getSensitivityAlgoGold <- function(alpha, alphaOut, gold) {
...
#     mean((resTes.adj[[i]][[algo]] < alpha)[!is.na(sigHeldout)])
#   })))
# }
...
colnames(sensMat) = c("DESeq2","edgeR","voom","GLMM","heldout")
...
#     if (sum(sigTest) == 0) return(0)
#     mean((resHeldout.adj[[i]][[gold]] < alphaOut)[!is.na(sigTest)])
#   })))
# }
precMat <- do.call(rbind,lapply(namesAlgos, function(algo) {
    res <- getPrecisionAlgoGold(alpha = .1,alphaOut = .1,gold = algo)
    data.frame(res,heldout=rep(algo,nrow(res)))
}))
colnames(precMat) = c("DESeq2","edgeR","voom","GLMM","heldout")
...
p <- ggplot(d, aes(x=evaluation,y=sensitivity,color=evaluation)) + scale_y_continuous(trans="log") + ylim(c(0,0.2))
p + scale_color_manual(values = cols) + geom_boxplot(outlier.colour=rgb(0,0,0,0)) + 
  theme_bw() + facet_wrap(~ verification, nrow = 1) + 
  geom_point(position = position_jitter(w = 0.1, h = 0), color="grey50", size=1) +
...
p <- ggplot(d, aes(x=evaluation,y=precision,color=evaluation)) + scale_y_continuous(trans = "exp") #+ ylim(c(0.5,1))
p + scale_color_manual(values = cols) + geom_boxplot(outlier.colour=rgb(0,0,0,0)) + theme_bw() + facet_wrap(~ verification, nrow=1) +
    geom_point(position = position_jitter(w = 0.1, h = 0), color="grey50", size=1) +
...
fdrAtAlphaPlot$algorithm <- renameAtoB(fdrAtAlphaPlot$algorithm, "edgeR.robust", "edgeR")
...
for(gold in names(resTes.adj[[1]][,-1])){
  for(algo in names(resTes.adj[[1]][,-1])){

...
variable_name = "group"
NormIdx <- which(coldata$group == "normal")
TumorIdx <- which(coldata$group == "ALZ")
# half_1_2 <- round(c(length(NormIdx)/2, length(TumorIdx)/2))
# names(half_1_2) <- c(grp1_name,grp2_name)
# If one of index lists is bigger than the other, we select only the first n indexes for both lists
# So index_1 and index_2 lists are of equal lengths
...
printIdx <- function() {
  ...
  NormSub2 <- sample(index_1bis, size = 3, replace = FALSE)
...
write.table(randomSubsets,file="/blackhole/alessia/GLMM_article/robustness/ALZ_random_subsets_eval_veri.txt", 
            quote=FALSE,row.names=FALSE,col.names=FALSE)
...
grp2_name = "T_ALL"
variable_name = "group"
NormIdx <- which(coldata$group == "Thymocyte")
TumorIdx <- which(coldata$group == "T_ALL")
index_1 <- which(coldata[,variable_name] == grp1_name)
index_2 <- which(coldata[,variable_name] == grp2_name)
printIdx <- function() {
  NormSub <- sample(index_1, size = 5, replace = FALSE)
  tumorSub <- sample(index_2, size = 8, replace = FALSE)
  idx <- c(NormSub, tumorSub)
  
  index_1bis = index_1[-NormSub]
  index_2bis = index_2[-tumorSub]
  
  NormSub2 <- sample(index_1bis, size = 7, replace = FALSE)
  tumorSub2 <- sample(index_2bis, size = 12, replace = FALSE)
  idxbis = c(NormSub2, tumorSub2)
...
write.table(randomSubsets,file="/blackhole/alessia/CircModel/robustness/TALL_random_subsets_eval_veri.txt", 
            quote=FALSE,row.names=FALSE,col.names=FALSE)

...
# load(file = "/blackhole/alessia/CircModel/type_I_error_control/ALZ_typeIerrorGLM_GLMM_10rep.RData")
load(file = "/blackhole/alessia/CircModel/type_I_error_control/ALZ_typeIerrorGLM_GLMM_10repALL.RData")
load(file = "/blackhole/alessia/CircModel/type_I_error_control/ALZ_typeIerrorGLM_GLMM_20repALL.RData")
# load(file = "/blackhole/alessia/CircModel/type_I_error_control/TALL_typeIerrorGLM_GLMM_10repALL.RData")
dt.list = list()
dt.list2 = list()
for(x in 1:length(resMethods)) {dt.list[[x]] = melt(as.data.frame(resMethods[[x]]$rateComp), id = "alpha")}
for(x in 1:length(resMethods_ALZ_20)) {dt.list2[[x]] = melt(as.data.frame(resMethods_ALZ_20[[x]]$rateComp), id = "alpha")}

# for(x in 1:length(resMethods_TALL)) {dt.list2[[x]] = melt(as.data.frame(resMethods_TALL[[x]]$rateComp), id = "alpha")}

rate = rbind(#cbind(rbindlist(dt.list2, idcol = "rep"), dataset = "TALL"), 
  cbind(rbindlist(dt.list, idcol = "rep"), dataset = "ALZ"), 
  cbind(rbindlist(dt.list2, idcol = "rep"), dataset = "ALZ"))
colnames(rate) = c("Rep","alpha","Method","rate", "Dataset")
rate_df = as.data.frame(rate)
table(rate_df$Rep)
rate_df$Rep = rep(seq(1,30), each = 24)
rate_df$Method
# rateGLM = melt(rbindlist(lapply(resMethods, function(x) rbindlist(resRate, idcol = "Rep")), idcol = "DataSet"), id.vars = c("DataSet", "Rep"))
# colnames(rateGLM) = c("DataSet","Rep","Method","rate")
# rateGLMM = data.frame(rate = rbind(rbindlist(resRateGLMM)))
...
rate_df = rate_df %>% dplyr::filter(!Method%in%c("pvalue", "rateGLMM.adj"))
# rate_df$Method <- renameAtoB(rate_df$Method, "rateGLMM.adj", "GLMM-adj")
rate_df$Method<- renameAtoB(rate_df$Method, "rateGLMM", "GLMM")
rate_df$Method<- renameAtoB(rate_df$Method, "DESeq2-ZI", "DESeq2-ZeroInflated")
rate_df$Method<- renameAtoB(rate_df$Method, "DESeq2-glmGamPoi", "DESeq2-GamPoi")
...
rate_df$Rep
divergence_FDR_mean_melted <- ddply(.data = rate_df,.variables = ~ Dataset + Method + alpha,.fun = function(x){
  if(x$alpha == "0.1"){
    sum(abs(x[,"rate"]-0.1))/10
  } else if(x$alpha == "0.05"){
    sum(abs(x[,"rate"]-0.05))/10
  }
})
# rank the methods
ranked_divergence_FDR_mean_melted <- ddply(divergence_FDR_mean_melted,.variables = ~ Dataset + alpha, function(x) rank(x[,"V1"]))
colnames(ranked_divergence_FDR_mean_melted) <- c("Dataset","alpha",levels(divergence_FDR_mean_melted$Method))
FDR_divergence_summary <- data.frame(method = levels(divergence_FDR_mean_melted$Method), value = apply(ranked_divergence_FDR_mean_melted[,-(1:2)],2,mean))
saveRDS(object = FDR_divergence_summary,file = "/blackhole/alessia/CircModel/data/FDR_divergence_summary_ALZ.RDS")

rate_df_005 = rate_df %>% dplyr::filter(alpha=="0.05")
rate_df_01 = rate_df %>% dplyr::filter(alpha=="0.1")

png(paste0("/blackhole/alessia/CircModel/type_I_error_control/TIE_ALZ_30rep.png"), units = "cm", width = 20, height = 20, res = 100)
...
ggplot(data = rate_df, aes( color = Method)) +
    ...
    # geom_segment(aes(x = 2-0.5,xend = 2+0.5,y = 0.05,yend = 0.05), color = "red", lty = 2) +
    # geom_segment(aes(x = 3-0.5,xend = 3+0.5,y = 0.1,yend = 0.1), color = "red", lty = 2) +
    facet_wrap(Dataset ~ alpha) + 
    xlab(expression(Nominal~alpha)) + ylab(expression("Observed"~alpha)) +
    #ggtitle(label = "False discoveries for each method") + 
    scale_color_manual(values = cols) +
    theme_minimal() + 
    ggtitle(label = paste("False discoveries for ALZ data samples"),subtitle = "Raw p-values") +

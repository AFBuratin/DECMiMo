...
printIdx <- function() {
  NormSub <- sample(coldata$sample, size = 6, replace = FALSE)
  tumorSub <- sample(setdiff(coldata$sample, NormSub), size = 8, replace = FALSE)
  idx <- c(NormSub, tumorSub)
...
write.table(randomSubsets,file="/blackhole/alessia/CircModel/type_I_error_control/random_shuffle_TALL.txt",
            quote=FALSE,row.names=FALSE,col.names=FALSE)
